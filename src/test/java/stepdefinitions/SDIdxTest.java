...
public class SDIdxTest {
    ...
    @Then("user go to cart page, verify product {string} exist and update qty {string} and see total {string} and checkout item")
    public void user_go_to_cart_page_and_update_qty_and_checkout_item(String product, String qty, String total) {
        WebUI.clickButtonByLabel("Check Out");
        WebUI.delay(3);
        // WebUI.click(By.xpath("//a[contains(normalize-space(.), 'My Cart')]"));
        WebUI.verifyTextContainsByLocator(By.xpath("//div[@class='row']//h3/a[contains(normalize-space(.), '" + product + "')]"), product);
        WebUI.setText(By.xpath("//h3[a[contains(normalize-space(.), '" + product + "')]]/ancestor::div[@class='row']//input[@name='updates[]']"), qty);
        WebUI.delay(1);
        ...
        int gTotal = Integer.parseInt(qty) * (int) Double.parseDouble(getBaseValue);
        System.out.println("gTotal: " + gTotal);
